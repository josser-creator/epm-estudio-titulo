...
class ConectaAPIService(BaseService):
    ...
    def _get_token(self) -> str:
        ...
        """
        # Verificar si el token actual aún es válido (con margen)
        ...
        try:
            ...
            # Calcular expiración (restando 5 minutos para margen). Clamp para evitar negativos.
            self._token_expires_at = datetime.utcnow() + timedelta(seconds=max(expires_in - 300, 0))
    ...
    def enviar_resultado(self, caso_id: str, datos_extraidos: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self._log_error("Error enviando a Conecta", error=e)
